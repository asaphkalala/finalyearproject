...
{
	indicesCount = 36; // calcuation: 6 faces * 2 triangles * 3 vertices
	vertexCount = 24; // caluclation: 6 faces * 4 vertices
	constructModel(minBound, maxBound); // Constructs the cube model
}
...
void Cube::draw() const
{
	...
	glDrawElements(GL_TRIANGLES, indicesCount, GL_UNSIGNED_INT, nullptr); // Draws the cube 
...
void Cube::constructModel(const glm::vec3& minBound, const glm::vec3& maxBound) 
{
	...
	// Creates the normals of the cube
	...
	const glm::vec3 normals[] = {
	 ...
	 // Creates the texture coordinates of the cube
	...
	{
		data.pushBack(vertices[i].x);
		data.pushBack(vertices[i].y);
		data.pushBack(vertices[i].z);
		data.pushBack(normals[i].x);
		data.pushBack(normals[i].y);
		data.pushBack(normals[i].z);
		data.pushBack(texCoords[i].x);
		data.pushBack(texCoords[i].y);
	}

	// Creates the indices array object (defines the triangles) 
	glGenBuffers(1, &ebo); 
	glBindVertexArray(vao);
